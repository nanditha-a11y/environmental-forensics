    ...
    <AnimatePresence>
      {open && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 sm:p-6">
          {/* Lightened overlay so background FX show through */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            ...
            onClick={onClose}
            className="absolute inset-0 bg-night-950/40 backdrop-blur-md"
          />
          {/* Replaced glass-strong with standard glass + semi-transparent border */}
          <motion.div
            role="dialog"
            aria-modal="true"
            ...
            className={`glass relative w-full ${wide ? 'max-w-2xl' : 'max-w-lg'} rounded-2xl overflow-hidden border border-white/10 shadow-[0_16px_44px_-14px_rgba(2,8,18,0.8)]`}
          >

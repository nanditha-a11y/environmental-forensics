...
function AlertDetail({ alert, onClose }: { alert: AlertItem; onClose: () => void }) {
  ...
  return (
    <Modal
      ...
      icon="alert"
      accent={alert.severity === 'high' ? 'border-red-400/25 bg-red-400/10 text-red-300' : 'border-amber-400/25 bg-amber-400/10 text-amber-300'}
    >
      <div className="space-y-4">
        ...
        {acknowledged ? (
          ...
          <Button
            variant={alert.severity === 'high' ? 'danger' : 'emerald'}
            className="w-full"
...
export function RecentAlerts() {
  ...
  // Live API query integration
  const { data: apiAlerts = [], isLoading, isError, error, refetch } = useAlerts();

  const q = query.trim().toLowerCase();
  const filtered = q
    ? apiAlerts.filter((a: AlertItem) => `${a.title || ''} ${a.source || ''}`.toLowerCase().includes(q))
    : apiAlerts;
    ...
    <GlassCard className="flex h-full flex-col overflow-hidden">
      <div id="recent-alerts" className="flex items-center justify-between border-b border-white/[0.06] px-4 py-3.5 scroll-mt-6">
        <div>
          ...
          <p className="mt-0.5 text-[11px] text-slate-500">Live detection stream · {filtered.length} new</p>
        </div>
          ...
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center gap-2 px-4 py-8 text-center">
              ...
              <p className="text-[12.5px] text-slate-400">
                {q ? `No alerts match “${query}”.` : 'No recent alerts logged.'}
              </p>
            </div>
          ) : (
            <AnimatePresence mode="popLayout">
              {filtered.map((alert: AlertItem, i: number) => (
                <motion.button
                  key={alert.id}
                  layout
